...
- [ ] 모바일: 메시지 상세(message/[id])에서 알람 설정 바로가기 (웹 상세 모달과 동일 기능)
- [ ] 웹: 알람 생성 화면에서 메시지 목록 검색/필터 추가 (많은 메시지 중 선택 편의)
- [ ] 백엔드: DELETE /api/voice/:id에서 연관 메시지 존재 시 경고 응답 (409 + force=true, message DELETE 패턴과 동일)
- [ ] 모바일: 친구 상세 화면에서 선물 보내기 바로가기 버튼 추가
- [ ] 백엔드: stats.test.ts 신규 (stats 라우트 테스트)
