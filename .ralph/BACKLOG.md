...
- [ ] 모바일/웹: 메시지 삭제 시 409 응답 처리 (연관 알람 경고 confirm dialog)
- [ ] 웹: 음성 프로필 상세 모달 (해당 음성의 메시지/알람 목록, 모바일과 동일)
- [ ] 모바일: 알람 편집 화면에서 GET /api/alarm/:id 활용 (현재 params로 전달 → API 조회로 개선)
- [ ] 백엔드: GET /api/voice/:id/stats 엔드포인트 (해당 음성의 메시지 수, 알람 수 한 번에 반환)
