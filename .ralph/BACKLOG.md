...
- [ ] 모바일: 선물 받은 화면에서 보낸 사람의 노트 표시 (현재 note 필드 미표시)
- [ ] 웹: GiftsPage에서 노트 표시 (보낸/받은 선물 모두)
- [ ] 백엔드: GET /api/alarm/:id 단일 알람 조회 엔드포인트 추가 (편집 화면용)
- [ ] 모바일: 음성 프로필 상세 화면 (해당 음성으로 만든 메시지/알람 목록)
