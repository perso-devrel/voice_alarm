...
- [ ] 모바일: 메시지 라이브러리 탭에서 스와이프 삭제 제스처 지원
- [ ] 백엔드: GET /api/alarm에 페이지네이션 추가 (friend/gift와 동일 패턴)
- [ ] 웹: 메시지 목록에서 삭제 시 낙관적 업데이트 (optimistic update)
- [ ] 모바일: 알람 생성 화면에서 프리셋 메시지 선택 지원 (빈 메시지 → 프리셋 텍스트 선택 → TTS 생성 → 알람 설정)
